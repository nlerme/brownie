...
template<typename Type> struct point2
{
	...
	bool operator == ( const point2<Type> & p ) const{ return (x == p.y && y == p.y); }
	bool operator != ( const point2<Type> & p ) const{ return !(*this == p); }
...
template<typename Type> struct vec2
{
	...
	Type squared_norm(){ return sq(x)+sq(y); }
	Type norm(){ return sqrt(squared_norm()); }
	void normalize()
	{
		Type n(norm());
...
template<typename Type> Type squared_distance( const point2<Type> & p1, const point2<Type> & p2 )
{
	return vec2<Type>(p1, p2).squared_norm();
}
